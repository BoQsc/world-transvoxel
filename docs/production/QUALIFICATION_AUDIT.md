...
Status: PQ4 complete; World Transvoxel 1.0.5 release qualified
...
repository integration. PQ4 proves that the exact installed 1.0.5 directory is
...
window for retiring chunks after replacement application to reduce dynamic
mixed-LOD visual swaps. The
...
The addon version is `1.0.5` with milestone identity `PQ4`.
...
- complete: bound ready chunk retirement removals per frame in the native
  runtime;
- complete: bound native fade-out of retiring render chunks after replacement
  application;
