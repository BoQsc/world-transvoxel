...
WtBalancedLodPlannerStatus WtBalancedLodPlanner::refine_interaction_shell(
	const WtBalancedLodPlan &current,
	...
	if (current.entries.empty() || interaction_keys.empty()) {
		output = current;
		return WtBalancedLodPlannerStatus::Ok;
	}
	std::vector<WtChunkKey> focus = interaction_keys;
	...
	for (const WtChunkKey &key : focus) {
		...
		for (;;) {
			...
			if (leaf == leaves.end()) break;
			if (leaf->lod == 0) break;
			const std::size_t leaf_index = static_cast<std::size_t>(
				std::distance(leaves.begin(), leaf)
			);
			const WtBalancedLodPlannerStatus status =
				refine_leaf(leaves, leaf_index);
			if (status != WtBalancedLodPlannerStatus::Ok) return status;
		}
	}
	WtLodMap map(active_capacity_);
	const WtBalancedLodPlannerStatus balance_status = balance(leaves, map);
	if (balance_status != WtBalancedLodPlannerStatus::Ok) {
		return balance_status;
	}
	output.entries = map.get_entries();
	...
	for (const WtLodMapEntry &entry : output.entries) {
		if (!append_staged_demand(
				entry.key, current, current, output.demands
			)) {
			output.clear();
			return WtBalancedLodPlannerStatus::InvalidLodMap;
		}
		if (std::binary_search(focus.begin(), focus.end(), entry.key)) {
			...
			demand.priority = std::max(demand.priority, interaction_priority);
