...
func _run_test() -> void:
	...
	if terrain.get_render_resource_count() != 1 or \
			terrain.get_collision_resource_count() != 1:
		return _fail("resource replacement did not direct-swap render/collision resources")
	for frame in range(2):
		await process_frame
	if terrain.get_render_resource_count() != 1 or \
			terrain.get_collision_resource_count() != 1:
		return _fail("resource replacement did not remain settled after direct swap")
