...
from bootstrap_toolchain import GODOT_CPP_REVISION, ZIG_VERSION
from wt_script_common import REPO_ROOT, native_tool_path, require_supported_python
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Bake dense scalar/material volumes with native World Transvoxel code."
    )
    ...
    executable = native_tool_path(arguments.configuration, "wt_bake_tool")
    if not executable.is_file():
        raise RuntimeError(
            "wt_bake_tool is not built. Run python scripts/build.py first."
        )
    command = [
        str(executable),
        "dense",
